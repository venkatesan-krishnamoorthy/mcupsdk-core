...
#ifdef TINYUSB_INTEGRATION
    tusb_speed_t t_speed;
    dcd_event_t event = { .rhport = 0, .event_id = DCD_EVENT_BUS_RESET };
    /* pass the acutal speed to TinyUSB driver */
    switch (speed)
    {
        case USB_SPEED_LOW:
        t_speed = TUSB_SPEED_LOW;
        break;
        case USB_SPEED_FULL:
        t_speed = TUSB_SPEED_FULL;
        break;
        case USB_SPEED_HIGH:
        t_speed = TUSB_SPEED_HIGH;
        break;
        default:
        t_speed = TUSB_SPEED_HIGH;
        break;
    }
    event.bus_reset.speed = t_speed;
    TU_LOG2("Application: gadget connet at %d speed\n",  t_speed);
...
int dwc_usb3_gadget_disconnect(volatile dwc_usb3_pcd_t *pcd)
{
	dwc_debug1(pcd->usb3_dev, "%s()\n", __func__);
#ifdef TINYUSB_INTEGRATION
    dcd_event_t event = { .rhport = 0, .event_id = DCD_EVENT_UNPLUGGED };
    TU_LOG2("%s()\n", __func__);
    /* send bus reset event to TinyUSB stack */
    dcd_event_handler(&event, true);
#else
...
#ifdef TINYUSB_INTEGRATION
    dcd_event_t event = { .rhport = 0, .event_id = DCD_EVENT_RESUME };
    TU_LOG2("%s()\n", __func__);
    /* send bus reset event to TinyUSB stack */
    dcd_event_handler(&event, true);
#endif
...
int dwc_usb3_gadget_setup(volatile dwc_usb3_pcd_t *pcd, usb_device_request_t *ctrl)
{
    ...
    dwc_debug(pcd->usb3_dev, "setup: %02x, %02x, %04x, %04x, %04x\n",
            ctrl->bmRequestType,
            ctrl->bRequest,
            UGETW(ctrl->wValue),
            UGETW(ctrl->wIndex),
...
void *dwc_usb3_gadget_alloc_dma(volatile dwc_usb3_pcd_ep_t *pcd_ep, int size, dwc_dma_t *mem_dma_ret)
{
	...
	if (pcd_ep->dwc_ep.num == 0U) {
		dwc_error0(pcd_ep->dwc_ep.pcd->usb3_dev, "USB epnum is 0 for pcd_ep\n");
		return NULL;
	}

#ifdef LINUXTEST
	mem = dma_alloc_coherent(&g_usb3_dev.pcidev->dev, size, &mem_dma, GFP_ATOMIC | GFP_DMA32);
	if (!mem)
		return NULL;
#else
	if (pcd_ep->dwc_ep.is_in > 0U) {
		mem = (void *)&g_in_trb_pool[pcd_ep->dwc_ep.num - 1U];
	}
    else {
		mem = (void *)&g_out_trb_pool[pcd_ep->dwc_ep.num - 1U];
	}
...
#ifdef LINUXTEST
	dma_free_coherent(&g_usb3_dev.pcidev->dev, size, mem, mem_dma);
#endif
}
...
void dwc_usb3_gadget_start_next_request(volatile dwc_usb3_pcd_t *pcd, volatile dwc_usb3_pcd_ep_t *pcd_ep)
{
	...
	DWC_CIRCLEQ_FOREACH(pcd_req, &pcd_ep->dwc_ep.queue, entry) {
		if ((pcd_req->dwc_req.flags & DWC_PCD_REQ_STARTED) == 0U) {
...
void dwc_usb3_gadget_isoc_ep_start(volatile dwc_usb3_pcd_t *pcd, volatile dwc_usb3_pcd_ep_t *pcd_ep, u32 event)
{
	...
	if (dwc_usb3_pcd_isoc_ep_hiber_restart(pcd, pcd_ep) != 0) {
		return;
	}
	...
	DWC_CIRCLEQ_FOREACH(pcd_req, &pcd_ep->dwc_ep.queue, entry) {
		if ((pcd_req->dwc_req.flags & DWC_PCD_REQ_STARTED) > 0U) {
			/* pcd_req = NULL;
			 * K3ROM: Modifying the above line with "continue" to fix the klocwork
			 * error. Since the structure is set to NULL is being deferenced in
			 * the for loop. Ideally we want to traverse the queue for all non
			 */
			continue;
		}
		else {
...
volatile usb_ep_t *dwc_usb3_ep_enable(struct dwc_usb3_device *usb3_dev, const void *epdesc, const void *epcomp)
{
	...
	if (ep_num < 1U || ep_num >= DWC_MAX_EPS)
	{
		return NULL;
	}

	if (ep_dir == UE_DIR_IN) {
		pcd_ep = pcd->in_ep[ep_num - 1U];
	} else {
		pcd_ep = pcd->out_ep[ep_num - 1U];
	}
	...
	desc = dwc_usb3_pcd_trb_alloc(pcd_ep, num_trbs, ep_type, ep_desc->bInterval - 1U,
				      link, &desc_dma);
	if (!desc) {
		return NULL;
	}
	...
	usb_ep->desc = epdesc;
	usb_ep->address = ep_desc->bEndpointAddress;
...
int dwc_usb3_close_all_ep(volatile struct dwc_usb3_device *usb3_dev)
{
    ...
    /* disable any active EP and free its trb */
    for (i = 0; i < pcd->num_in_eps; i++) {
        pcd_ep = pcd->in_ep[i];
        if (pcd_ep->dwc_ep.active > 0U) {
            retval += dwc_usb3_ep_disable(usb3_dev, &pcd_ep->usb_ep);
        }
    }
...
usb_request_t *dwc_usb3_alloc_request(struct dwc_usb3_device *usb3_dev, volatile usb_ep_t *usb_ep)
{
	...
	dwc_info(usb3_dev, "%s(%lx)\n", __func__, (unsigned long)usb_ep);

	for (req_idx = 0; req_idx < 32U; req_idx++) {
		if ((g_pcd_req_bm & (1U << req_idx)) > 0U) {
			g_pcd_req_bm &= ~(1U << req_idx);
...
int dwc_usb3_ep_queue(struct dwc_usb3_device *usb3_dev, volatile usb_ep_t *usb_ep, usb_request_t *usb_req)
{
	...
	if (!DWC_CIRCLEQ_EMPTY(&pcd_ep->dwc_ep.queue)) {
		...
		if (pcd_ep->dwc_ep.stopped == 0U && pcd_ep->dwc_ep.type == UE_ISOCHRONOUS &&
		    pcd_ep->dwc_ep.desc_avail > 0 && pcd_ep->dwc_ep.xfer_started > 0U) {
			goto do_start;
		}
	} else
#endif
	if ((pcd_ep->dwc_ep.stopped > 0U) || (pcd_ep != pcd->ep0 && pcd_ep->dwc_ep.desc_avail <= 0) ||
	    (pcd_ep->dwc_ep.type == UE_ISOCHRONOUS && pcd_ep->dwc_ep.xfer_started == 0U)) {
...
int dwc_usb3_ep_dequeue(struct dwc_usb3_device *usb3_dev, usb_ep_t *usb_ep, usb_request_t *usb_req)
{
	...
	if (dwc_usb3_pcd_ep_num(pcd_ep) != 0U && !pcd_ep->dwc_ep.usb_ep_desc) {
