...
const static usb_dev_cap_ss_usb_t cap2_desc = {
	...
	UCONSTW(256U),				/* wU2DevExitLat (256 us) */
};
...
static void do_set_address(volatile dwc_usb3_pcd_t *pcd)
{
	...
	if (ctrl.bmRequestType == ((uByte) UT_DEVICE)) {
#ifdef DEBUG_EP0
...
static void ep0_do_stall(volatile dwc_usb3_pcd_t *pcd, int err_val)
{
	usb_device_request_t ctrl __attribute__((unused)) = pcd->ep0_setup_pkt->req;
...
static void do_clear_halt(volatile dwc_usb3_pcd_t *pcd, volatile dwc_usb3_pcd_ep_t *ep)
{
	...
	if (ep->dwc_ep.stall_clear_flag == 0U) {
...
static void do_get_descriptor(volatile dwc_usb3_pcd_t *pcd)
{
...
#if !defined(__linux__) || !defined(DWC_BOS_IN_GADGET)  /* TI modification */
    usb_dev_cap_20_ext_desc_t* local_cap21_desc;
#endif
	...
	case UDESC_BOS:
		dwc_debug0(pcd->usb3_dev, "\n\n\n\nGET_DESCRIPTOR(BOS)\n\n\n");
		if (pcd->speed != USB_SPEED_SUPER &&
		    (pcd->usb3_dev->core_params->nobos) > 0) {
        ...
        /* TI modification - since we changed the descriptor declaration to "const" so that the
         * descriptor does not fall into cinit section, we have to deal with a local copy of cap1_desc */
        local_cap21_desc = (usb_dev_cap_20_ext_desc_t*)buf;
        memcpy(local_cap21_desc, &cap1_desc, sizeof(usb_dev_cap_20_ext_desc_t));

		if (pcd->usb3_dev->core_params->besl > 0) {
			u32 d = UGETDW(local_cap21_desc->bmAttributes);
			...
			d |= ((u32) pcd->usb3_dev->core_params->baseline_besl) <<
				USB_20_EXT_BASELINE_BESL_SHIFT;
			d |= ((u32) pcd->usb3_dev->core_params->deep_besl) <<
				USB_20_EXT_DEEP_BESL_SHIFT;
			USETDW(local_cap21_desc->bmAttributes, d);
		}

		if (pcd->usb3_dev->core_params->lpmctl == 0) {
			USETDW(local_cap21_desc->bmAttributes, (u16)0);
		}

		//memcpy(buf, &cap1_desc, sizeof(cap1_desc)); // TI - we already copied above.
        //end of TI modification
...
static void do_get_status(volatile dwc_usb3_pcd_t *pcd)
{
			...
			*status = 1; /* Self powered */
			...
			if (pcd->speed == USB_SPEED_SUPER) {
				if (pcd->state == DWC_STATE_CONFIGURED) {
					if (dwc_usb3_u1_enabled(pcd) != 0) {
						*status |= 1U << 2;
					}

					if (dwc_usb3_u2_enabled(pcd) != 0) {
						*status |= 1U << 3;
					}
	...
	case UT_INTERFACE:
		*status = 0;
		if (pcd->usb3_dev->core_params->wakeup > 0) {
			*status |= 1U;
		}
		*status |= pcd->remote_wakeup_enable << 1;
...
static void do_set_feature(volatile dwc_usb3_pcd_t *pcd)
{
	...
	case UT_DEVICE:
		...
		case UF_U1_ENABLE:
			...
			if ((pcd->usb3_dev->core_params->pwrctl & 1) > 0) {
		...
		case UF_U2_ENABLE:
			...
			if ((pcd->usb3_dev->core_params->pwrctl & 2) > 0) {
...
static void dwc_usb3_do_setup(volatile dwc_usb3_pcd_t *pcd)
{
	...
	if ((ctrl.bmRequestType & UE_DIR_IN) > 0U) {
		ep0->dwc_ep.is_in = 1;
 ...
 */
void dwc_usb3_print_ep0_state(volatile dwc_usb3_pcd_t *pcd)
{
#ifdef DEBUG
	char *str;
				...
				dwc_usb3_dma_desc_t *desc, int status)
{
    ...
    /* NOTES: Since EP0 doesn't support multiple requests and thus the defined default call back
     * mechanism  doesn't work here.  Adding custom code here so that a call back is called when
     * data phase is done. Don't touch any other codes */
	if (pcd->ep0state == EP0_OUT_DATA_PHASE ||
	    pcd->ep0state == EP0_IN_DATA_PHASE) {
        //#warn "modifying Synopsys code for customized call back function during EP0 data phase"
		if (pcd->ep0_req->usb_req.complete != 0) {
		    pcd->ep0_req->usb_req.status = status;
		    pcd->ep0_req->usb_req.actual = pcd->ep0_req->dwc_req.actual;
            pcd->ep0_req->usb_req.complete(&(pcd->ep0->usb_ep), &(pcd->ep0_req->usb_req));
        }
    }
    /* END of customized modification */

	if (pcd->ep0state == EP0_OUT_STATUS_PHASE ||
	    pcd->ep0state == EP0_IN_STATUS_PHASE) {
		is_last = 1;

	} else if (ep->dwc_ep.is_in > 0U) {
#ifdef DEBUG_EP0
		...
		if (dwc_usb3_get_xfercnt(desc) == 0U) {
			...
			if ((req->dwc_req.flags & DWC_PCD_REQ_ZERO) > 0U) {
#ifdef DEBUG_EP0
	...
	if (pcd->ep0_status_pending == 0U)
	{
		if (ep->dwc_ep.is_in > 0U){
			pcd->ep0->dwc_ep.is_in = 0;
			pcd->ep0state = EP0_OUT_WAIT_GADGET;
		}
		else {
			pcd->ep0->dwc_ep.is_in = 1;
			pcd->ep0state = EP0_IN_WAIT_GADGET;
				...
				u32 event)
{
	...
	case EP0_IN_DATA_PHASE:
		...
		if ((status & DWC_TRBRSP_SETUP_PEND) > 0U) {
		...
		} else if (ep0->dwc_ep.send_zlp > 0U) {
